...
yesBtn.MouseButton1Click:Connect(function()
    frame.Visible = false
    showDevicePopup(deviceCallback) -- Continue to device popup
end)
    -- Place your next popup or GUI logic here.
end)
...
function showDevicePopup(callback)
    local player = game:GetService("Players").LocalPlayer
    local TweenService = game:GetService("TweenService")
    local UserInputService = game:GetService("UserInputService")
    ...
    okBtn.MouseButton1Click:Connect(function()
        ...
        gui:Destroy()
        if callback then callback(selectedDevice) end

        -- Handle device-specific toggling
        if selectedDevice == "Mobile" then
            uiToggle.Visible = true
            -- The rest is managed by your uiToggle.MouseButton1Click logic (already in your script)
        else
            uiToggle.Visible = false
            -- Setup B key toggle for PC
            local connection
